...
SELECT * FROM artikel 
	JOIN kategorie ON artikel.fk_kategorie = kategorie.kat_nr
    WHERE kategorie.kategoriebezeichnung = "Monitor"
    ORDER BY artikel.einzelpreis DESC;
...
-- Jede Kategorie soll nur einmal betrachtet werden (Hinweis: daher DISTINCT wichtig, wenn nur einmal betrachtet werden soll!).
SELECT DISTINCT kategorie.kategoriebezeichnung AS "Kategorien" FROM auftrag 
	JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
	JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
	JOIN kategorie ON kategorie.kat_nr = artikel.fk_kategorie
...
SELECT auftrag.fk_kunde AS "Kundennummer", auftrag.auft_nr AS "Auftragsnummer", bestellposition.fk_artikel AS "Artikel", bestellposition.anzahl AS "Bestellmenge", auftrag.bestelldat AS "Bestelldatum" FROM auftrag 
	JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
		WHERE auftrag.fk_kunde = 111 
			ORDER BY auftrag.bestelldat ASC;

-- Abfrage 6
-- Wie viele Kunden kommen aus dem PLZ-Gebiet 5?
SELECT COUNT(*) AS "Anzahl Kunden aus dem PLZ-Gebiet 5" FROM kunde 
	WHERE LEFT(plz, 1) = 5;
...
SELECT SUM(artikel.einzelpreis * bestellposition.anzahl) AS "Gesamtumsatz in Essen" FROM kunde
	JOIN auftrag ON auftrag.fk_kunde = kunde.kd_nr
    JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
    JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
	...
	JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
		WHERE bestellposition.fk_artikel = 555
			GROUP BY auftrag.auft_nr 
				ORDER BY MAX(DISTINCT bestellposition.anzahl) DESC LIMIT 1;
...
SELECT LEFT(kunde.plz, 2) AS "PLZ-Gebiet", COUNT(kunde.kd_nr) AS "Anzahl Kunden" FROM kunde
	GROUP BY LEFT(kunde.plz, 2) 
		ORDER BY COUNT(kunde.kd_nr) DESC LIMIT 3;
...
SELECT auftrag.fk_kunde AS "Kundennummer" FROM auftrag
	WHERE auftrag.bestelldat IN
        ...
        (SELECT MAX(auftrag.bestelldat) FROM auftrag 
			WHERE auftrag.fk_kunde = 400)
	
    AND auftrag.fk_kunde != 400
...
SELECT LEFT(kunde.plz, 1) AS "PLZ-Gebiet", SUM(bestellposition.anzahl * artikel.einzelpreis) AS "Gesamtumsatz" FROM kunde
    ...
    JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
		GROUP BY LEFT(kunde.plz, 1)
			ORDER BY SUM(bestellposition.anzahl * artikel.einzelpreis) ASC;
...
SELECT kategorie.kat_nr AS "Kategorie", kategorie.kategoriebezeichnung AS "Bezeichnung", COUNT(artikel.art_nr) AS "Anzahl Artikel" FROM kategorie
	JOIN artikel ON artikel.FK_kategorie = kategorie.kat_nr
		GROUP BY kategorie.kat_nr, kategorie.kategoriebezeichnung 
			ORDER BY kategorie.kat_nr ASC;
...
-- Abfrage 16 (K 95% Wahrscheinlichkeit)
-- Zeigen Sie alle Aufträge an, zu denen es keine Daten in der Bestellpositionentabelle gibt (Hinweis: daher Left-Join!).
...
SELECT COUNT(DISTINCT shop.fk_stadt) AS "Anzahl Städte mit mindestens einem Shop des Typs 'Großhandel'" FROM shop
	JOIN shoptyp ON shoptyp.typ_nr = shop.fk_shoptyp
...
SELECT auftrag.auft_nr AS "Auftragsnummer", SUM(bestellposition.anzahl * artikel.einzelpreis) AS "Gesamtwert" FROM auftrag
    ...
    JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
		WHERE bestellposition.anzahl * artikel.einzelpreis IS NOT NULL AND auftrag.bestelldat between "2020-01-01" AND "2020-12-31"
			GROUP BY auftrag.auft_nr 
				ORDER BY SUM(bestellposition.anzahl * artikel.einzelpreis) ASC LIMIT 1;
...
SELECT DISTINCT auftrag.fk_kunde AS "Kundennummer", kunde.vorname AS "Vorname", kunde.nachname AS "Nachname" FROM auftrag
	JOIN kunde ON kunde.kd_nr = auftrag.fk_kunde
...
SELECT LEFT(kunde.plz, 1) AS "PLZ-Gebiet", AVG(DATEDIFF(auftrag.lieferdat, auftrag.bestelldat)) AS "Durchschnittliche Lieferzeit [Tage]" FROM kunde
	JOIN auftrag ON auftrag.fk_kunde = kunde.kd_nr
		WHERE LEFT(kunde.plz, 1) IN ("4", "8")
			GROUP BY LEFT(kunde.plz, 1)
...
-- Abfrage 29 (K)
-- Welche Kunden haben im nächsten Monat Geburtstag? Die Abfrage soll allgemeingültig sein und sich immer auf 
-- das aktuelle Datum beziehen! Die Ausgabe soll sortiert nach der Tageszahl erfolgen.
...
SELECT kunde.kd_nr, kunde.geburtsdatum FROM kunde
...
-- Hinweis: Lösung mit WHERE ... anstelle von HAVING ... wäre genauso möglich
SELECT shop.shop_nr AS "Shop-Nummer", shop.fk_shoptyp AS "Shop-Typ", shop.strasse AS "Straße", shop.plz AS "PLZ", stadt.stadt AS "Stadt" FROM shop
	JOIN stadt ON stadt.stadt_nr = shop.fk_stadt
	JOIN shoptyp ON shoptyp.typ_nr = shop.fk_shoptyp
    JOIN auftrag ON auftrag.fk_shop = shop.shop_nr
    JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
    JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
	JOIN kategorie ON kategorie.kat_nr = artikel.fk_kategorie
		GROUP BY shop.shop_nr, shop.fk_shoptyp, shop.strasse, shop.plz, stadt.stadt, kategorie.kategoriebezeichnung
...
SELECT LEFT(kunde.plz, 1) AS "PLZ-Gebiet", SUM(artikel.gewicht * bestellposition.anzahl) / COUNT(DISTINCT auftrag.auft_nr) AS "Durchschnittsgewicht pro Auftrag" FROM kunde
	JOIN auftrag ON auftrag.fk_kunde = kunde.kd_nr
    JOIN bestellposition ON bestellposition.fk_auftrag = auftrag.auft_nr
    JOIN artikel ON artikel.art_nr = bestellposition.fk_artikel
		GROUP BY LEFT(kunde.plz, 1)
...
SELECT kunde.ort AS "Top 5-Orte mit der kürzesten Lieferzeit", SUM(DATEDIFF(auftrag.lieferdat, auftrag.bestelldat)) / COUNT(DISTINCT auftrag.auft_nr) AS "Durchschnittliche Lieferzeit" FROM kunde
	JOIN auftrag ON auftrag.fk_kunde = kunde.kd_nr
		GROUP BY kunde.ort
...
SELECT artikel.art_nr, artikel.einzelpreis FROM artikel
	WHERE artikel.einzelpreis > 
    	
			(SELECT AVG(artikel.einzelpreis) FROM artikel)
		
        ORDER BY artikel.art_nr ASC;
...
SELECT kategorie.kategoriebezeichnung AS "Kategoriebezeichnung", COUNT(A.art_nr) AS "Artikel, teurer als Durchschnitt in jeweiliger Kategorie" FROM kategorie
	JOIN artikel AS A ON A.fk_kategorie = kategorie.kat_nr
		WHERE A.einzelpreis > 
...
SELECT kunde.kd_nr AS "Kundennummer", COUNT(DISTINCT auftrag.auft_nr) AS "Anzahl Aufträge" FROM kunde
	JOIN auftrag ON auftrag.fk_kunde = kunde.kd_nr
		WHERE auftrag.bestelldat LIKE "2019-12%"
...
-- Abfrage 46 (nicht K, aber eventuell andere Variante)
...
-- Abfrage 47 (schwerste Kategorie) *
...
-- Abfrage 49 (K in verschiedenen Varianten)
...
-- sollen den Wert 0 ausweisen (Hinweis: LEFT JOIN, um auch 0 zu finden)!
...
-- Hinweis: Auch Kunden ohne Aufträge sollen angezeigt werden (Hinweis: LEFT JOIN, um auch 0 zu finden).
SELECT kunde.kd_nr AS "Kundenummer", kunde.vorname AS "Vorname", kunde.nachname AS "Nachname", auftrag.auft_nr AS "Auftragsnummer", bestellposition.position AS "Bestellposition", bestellposition.fk_artikel AS "Artikel", bestellposition.anzahl AS "Menge" FROM kunde
...
SELECT * FROM kunde
	WHERE kunde.nachname IN 
        ...
        (SELECT kunde.nachname AS "Nachname" FROM kunde	
			GROUP BY kunde.nachname, kunde.vorname, kunde.ort 
				HAVING COUNT(DISTINCT kunde.kd_nr) > 1) 
                
	AND kunde.vorname IN 
		
        (SELECT kunde.vorname AS "Vorname" FROM kunde 
			GROUP BY kunde.nachname, kunde.vorname, kunde.ort 
				HAVING COUNT(DISTINCT kunde.kd_nr) > 1)
	
    AND kunde.ort IN 
    
		(SELECT kunde.ort AS "Ort" FROM kunde 
			GROUP BY kunde.nachname, kunde.vorname, kunde.ort 
				HAVING COUNT(DISTINCT kunde.kd_nr) > 1);
